...
it('lets 2 users exchange stars', async() => {
    ...
    await instance.createStar('Super Star', 7, {from: accounts[1]});
    await instance.createStar('Mega Star', 8, {from: accounts[2]});
...
it('lookUptokenIdToStarInfo test', async() => {
    let instance = await StarNotary.deployed();
    let starId = 6;
    let starName = 'Great Star';
    let user = accounts[1];
    await instance.createStar(starName, starId, {from: user});
    let greateStarName = await instance.lookUptokenIdToStarInfo(starId);
    assert.equal(greateStarName, starName);
});
